"""apply_message_entities 的 spoiler 行為單元測試（純函式，不需 DB）。
...
class ApplySpoilerEntitiesTests(unittest.TestCase):
    ...
    def test_returns_text_when_no_entities(self):
        self.assertEqual(apply_message_entities("hello", None), "hello")
        self.assertEqual(apply_message_entities("hello", []), "hello")

    def test_returns_text_when_no_spoiler_entities(self):
        entities = [{"type": "MessageEntityBold", "offset": 0, "length": 5}]
        self.assertEqual(apply_message_entities("hello", entities), "hello")

    def test_single_spoiler_ascii(self):
        entities = [{"type": "MessageEntitySpoiler", "offset": 6, "length": 5}]
        result = apply_message_entities("hello world!", entities)
    ...
    def test_single_spoiler_chinese(self):
        ...
        self.assertEqual(apply_message_entities(text, entities), "||明天會下雨||")
    ...
    def test_partial_chinese_spoiler(self):
        ...
        self.assertEqual(
            apply_message_entities(text, entities),
            "內鬼情報：||兇手是管家||",
        )
    ...
    def test_spoiler_with_emoji_surrogate_pair(self):
        ...
        self.assertEqual(apply_message_entities(text, entities), "a||😀||b")
