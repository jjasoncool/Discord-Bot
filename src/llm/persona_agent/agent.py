...
def inherit_keep_evidence(
    ...
    **revise 不沿用**：revise 是「不準確或已過時」的修正，常常是在推翻原句。真實案例：
    原句「會主動拋出職場與產業評論（台積電抓工時、台奴…）」被 revise 成「不再是高頻的
    產業評論發起者」，沿用的話「正事不做再那邊抓工時」這類撐原句的證據會掛到新句底下，
    剛好相反；而且舊證據會佔住前段，幾晚之後 revise 自己附的證據反而被擠掉。
